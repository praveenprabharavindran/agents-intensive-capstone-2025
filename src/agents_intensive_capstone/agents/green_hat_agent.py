...
DEFAULT_GREEN_HAT_PROMPT: str = (
    "You are the Green Hat. Focus on creativity, new ideas, and alternatives. "
    "How can we bypass constraints? What are some wild or out-of-the-box solutions?"
)


class GreenHatAgent(LlmAgent):
    """
    The *Green Hat* – the creative thinker that focuses on generating
    new ideas, alternatives, and out-of-the-box solutions.
    ...
    # -----------------------------------------------------------------------
    name: str = "GreenHatAgent"

    # These default values are used if a caller constructs GreenHatAgent
    # directly without the convenience constructor.
    instruction: str = DEFAULT_GREEN_HAT_PROMPT
    output_key: str = "green_hat_plan"

    # Filename that holds the prompt (used by the factory constructor).
    PROMPT_FILENAME: ClassVar[str] = "green_hat_prompt.txt"
    ...
    def create(
        ...
        **kwargs: Any,
        ...
        prompt_folder:
            Path to the directory that holds `green_hat_prompt.txt`.
        ...
        **kwargs:
        ...
        logger.info(
            "GreenHatAgent initialised (model=%s, tools=%d, prompt_source=%s)",
            type(agent.model).__name__,
            len(agent.tools or []),
            "file" if prompt_text != DEFAULT_GREEN_HAT_PROMPT else "default",
        )
    ...
    def _load_prompt(cls, prompt_folder: Path) -> str:
        ...
        try:
            ...
            if not text:
                logger.warning(
                    "Prompt file '%s' in '%s' is empty; using default GreenHat prompt.",
                    cls.PROMPT_FILENAME,
                    prompt_folder,
                )
                return cls._default_prompt()

            logger.debug("Loaded Green Hat prompt from %s", prompt_path)
            return text

        except FileNotFoundError:
            logger.warning(
                "Prompt file '%s' not found in '%s'; using default GreenHat prompt.",
                cls.PROMPT_FILENAME,
                prompt_folder,
            )
            return cls._default_prompt()
    ...
    def _default_prompt() -> str:
        ...
        return DEFAULT_GREEN_HAT_PROMPT
    ...
    def _build_tools(cls) -> List[Any]:
        ...
        Example:
            ...
            class CreativeGreenHatAgent(GreenHatAgent):
                @classmethod
                def _build_tools(cls) -> List[Tool]:
                    return [BrainstormingTool(), ConstraintRelaxerTool()]
        """
        # Example placeholder:
        # return [BrainstormingTool(), ConstraintRelaxerTool()]
        return []
